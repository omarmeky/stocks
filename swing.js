...
var calculateDays = function(dip, gain, quotes) {
    var days = 0;
    dip = (100 - dip) / 100;
    gain = 1 + (gain / 100);
    for (var i = quotes.length - 1; i > 0; i--) {
        if ((quotes[i].open * dip * 0.95) > quotes[i - 1].low)
          return -1;
        if (((quotes[i].open * dip) >= quotes[i].low) && ((quotes[i].open * dip * gain) <= quotes[i - 1].high)) {
          days++;
          i--;
        }
    }
...
var calculateMax = function(quotes) {
    ...
    for (var d = 0; d < 5.1; d+= 0.1) {
        for (var g = 0; g < 5.1; g+= 0.1) {
            var days = calculateDays(d, g, quotes);
            if (days === -1)
              return {
                dip: 0,
                gain: 0,
            ...
            if ((days > (quotes.length / 2.5)) && (newProfit > max.profit)) {
                max.dip = d;
                max.gain = g;
